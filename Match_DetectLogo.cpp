...
double MatchingHelper::whiteRatio(IplImage* img, cv::Rect rect){
...
double MatchingHelper::nextWhite(IplImage* img, cv::Rect rect){
	int height = rect.height;
	int width = rect.width;
	int step = img->widthStep;
	int channels  = img->nChannels;
...
double MatchingHelper::nextWhiteHorizonal(IplImage* img, cv::Rect rect){
	int height = rect.height;
	int width = rect.width;
	int step = img->widthStep;
	int channels  = img->nChannels;
...
cv::Rect MatchingHelper::findLogoRect(IplImage *g_pSrcImage){
...
double MatchingHelper::brightness(IplImage* img, int threshold){
		//Get Information of the image
...
double MatchingHelper::brightness(Mat img, int threshold){
	if(!img.data){
		return 0;
	}
